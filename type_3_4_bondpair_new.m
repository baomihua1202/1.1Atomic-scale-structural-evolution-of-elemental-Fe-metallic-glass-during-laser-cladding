clear,clc;
format short
fid=fopen('E:\laser-cladding\c-Fe-c-Fe_yure\1\c-Fe-c-Fe_1400K\1\annealing2.lammpstrj','rt'); 
for ii=1:1
...
d=zeros(size(dd,1),8);
for nn=1:size(dd,1)
    if ddd(nn,2)==3||ddd(nn,2)==4
        ww=nn;
        d(ww,:)=ddd(nn,:);
    end
    if ddd(nn,2)==1||ddd(nn,2)==2
        d(nn,:)=0.1;
    end
end
end
...
%A原子ID为1-9216
ab_b=zeros(size(dd,1)*size(dd,1),18);%找成A-B键的B原子ID，所有原子成键的ID都找到了,第一列为A原子ID，2-18列为B原子ID
ab_bxyz=zeros(size(dd,1)*size(dd,1),8);%将找到的成A-B键的B原子信息进行存储
for i=1:size(d,1)
...
for i=1:6
 ...
 qiuhe=sum(num_jk(:,3));
 for i=1:36
   num_jk (i,4)=(num_jk(i,3)/qiuhe)*100;
 end
